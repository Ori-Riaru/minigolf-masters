...
def slope(WINDOW, color, players, startX, startY, length, width, strengthX, strengthY):
    ...
    for ball in players:
        ...
        if (
            ball.x > startX
            and ball.x < startX + length
            and ball.y > startY
            and ball.y < startY + width
        ):
...
def stickyWallX(WINDOW, players, startX, startY, length, width):
    ...
    for ball in players:
        # Check if inside wall
        if (
            ball.x > startX - ball.r + 3
            and ball.x < startX + length + ball.r - 3
            and ball.y > startY
            and ball.y < startY + width
        ):
            # Move out of wall
...
class button:
    ...
    def isPressed(self):
        ...
        if (
            mouseY > self.y
            and mouseY < self.y + self.width
            and mouseX > self.x
            and mouseX < self.x + self.length
        ):
            # Change ui to target
            game.ui = self.target
    ...
    def update(self):
        ...
        if (
            ...
            pygame.draw.arc(
                WINDOW,
                self.color,
                (self.x - 50, self.y - 50, 100, 100),
                (self.angle - 0.05) * 3.1415 / 2,
                (self.angle + 0.05) * 3.1415 / 2,
                50,
            )
        ...
        # Draw ball
        pygame.draw.circle(
            WINDOW, (255, 255, 255), (round(self.x), round(self.y)), self.r
        )
...
while True:
    ...
    for event in pygame.event.get():
        ...
        # In game controls
            ...
            # Reset move values
                ...
                elif game.players >= 2 and (
                    event.key == pygame.K_a or event.key == pygame.K_d
                ):
                    players[1].angleChange = 0
                elif game.players >= 3 and (
                    event.key == pygame.K_j or event.key == pygame.K_l
                ):
                    players[2].angleChange = 0
                elif game.players >= 4 and (
                    event.key == pygame.K_KP4 or event.key == pygame.K_KP6
                ):
    ...
    if game.ui == 0:
        ...
        for ball in players:
            ...
            pygame.draw.rect(
                WINDOW, ball.color, (meterX, meterY, 21, (ball.charge / 4 - 100))
            )
            ...
            WINDOW.blit(
                hooge.render(
                    "P" + str(i) + ": " + str(ball.shotsTaken[game.level]), False, WHITE
                ),
                (shotsX + 33, 10),
            )
    ...
    # Score screen
        ...
        for ball in players:
            ...
            WINDOW.blit(
                hooge.render(str(sum(ball.shotsTaken)), False, BLACK),
                (1160, w * 50 + 150),
            )
            ...
            for i in range(20):
                WINDOW.blit(
                    hooge.render(str(ball.shotsTaken[i]), False, BLACK),
                    (i * 50 + 150, w * 50 + 150),
                )
        # Show player score for each player
        for z in range(20):
